...
const PriceGoingUpBanner = ({
  ...
  const timer = createTimeModel({
    initialTime: Math.abs(new Date(end_date).valueOf() - new Date().valueOf()),
    direction: "backward"
  });
  const { value } = useTimeModel(timer);
  return <>
        <div
      className={`prices-going-banner footer-sticky-banner ${className}`}
      style={{ display: bannerStatus ? "block" : "none" }}
    >
      <div className="banner-content">
        <div className="container">
          <div className="annual-text" dangerouslySetInnerHTML={{ __html: offer_name }} />
          <div className="right-block">
            <div className="current-price-group">
              <div className="box">
                <label dangerouslySetInnerHTML={{ __html: offer_desc }} />
              </div>
              <div className="box">
                <label>
                  Use Coupon <strong>{offer_coupon}</strong>
                </label>
              </div>
            </div>
            <div className="countdown-gorup">
              ...
              <a href={slug ? `/${slug}` : "#"}>
                <a className="btn-subscribe-now icon-font-arrow-right">Buy Now</a>
              </a>
            </div>
          </div>
        </div>
...
export default PriceGoingUpBanner;
